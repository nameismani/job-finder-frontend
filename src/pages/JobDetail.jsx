...
const JobDetail = () => {
  ...
  const handleDeletePost = async()=>{
    ...
    try{
   if(window.confirm("Delete job posts?")){
    const res = await apiRequest({
      ...
      <div className='w-full flex flex-col md:flex-row gap-10'>
        ...
        <div className='w-full h-fit md:w-2/3 2xl:2/4 px-5 py-10 md:px-10 shadow-md'>
          ...
          <div className='w-full flex flex-wrap md:flex-row gap-2 items-center justify-between my-10'>
            ...
            <div className='bg-[rgb(254,208,171)] w-40 h-16 px-6 rounded-lg flex flex-col items-center justify-center'>
              <span className='text-sm'>No. of Applicants</span>
              <p className='text-lg font-semibold text-gray-700'>
                {job?.application?.length || 0}K
              </p>
            </div>
